    ...
    <div className="relative w-full h-[420px] md:h-[480px] select-none" aria-hidden="true">
      ...
      <svg className="absolute inset-0 w-full h-full opacity-[0.15]" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
            <path d="M 40 0 L 0 0 0 40" fill="none" stroke="#2E7DC8" strokeWidth="0.5" />
          </pattern>
        </defs>
          ...
          <div className="space-y-1.5 mb-3">
            {[100, 80, 95, 65].map((w, i) => (
              <div key={i} className="h-1.5 rounded-full" style={{ width: `${w}%`, background: "rgba(255,255,255,0.08)" }}>
                <div className="h-full rounded-full" style={{ width: `${[68, 55, 80, 45][i]}%`, background: "rgba(46,125,200,0.7)" }} />
              </div>
            ))}
          </div>
          <div className="flex items-end gap-1 h-9">
            {[55, 80, 60, 92, 70, 84].map((h, i) => (
              <div
                ...
                style={{
                  height: `${h}%`,
                  background: i % 2 === 1 ? "rgba(46,125,200,0.7)" : "rgba(46,125,200,0.3)",
                }}
              />
            ))}
          </div>
      ...
      <div className="hero-float-b absolute top-12 left-[20%] md:left-[15%] z-10">
        <div className="glass-card w-64 rounded-sm overflow-hidden">
          <div className="h-[3px]" style={{ background: "linear-gradient(to right, #2E7DC8, #6EE7F9)" }} />
          <div className="p-5">
            ...
            <div className="flex items-end gap-3">
              <div>
                <p className="font-bold text-ink leading-none" style={{ fontSize: "2rem" }}>87%</p>
            ...
            {["01", "02", "03", "04"].map((n, i) => (
              <div key={n} className="flex items-center flex-1 last:flex-none">
                <div
                  ...
                  style={{
                    background: i === 0 ? "#2E7DC8" : "transparent",
                    border: i === 0 ? "none" : i === 1 ? "1px solid rgba(46,125,200,0.6)" : "1px solid rgba(255,255,255,0.12)",
                    color: i === 0 ? "#fff" : i === 1 ? "#2E7DC8" : "rgba(255,255,255,0.25)",
                  }}
                >
                  {n}
                </div>
                {i < 3 && <div className="flex-1 h-px mx-0.5" style={{ background: "rgba(255,255,255,0.1)" }} />}
              </div>
            ))}
          </div>
          <div className="flex justify-between">
            {["Diseño", "Campo", "Análisis", "Entrega"].map((s, i) => (
              <span key={s} className="text-[7px]" style={{ color: i === 1 ? "#2E7DC8" : "rgba(255,255,255,0.25)", fontWeight: i === 1 ? 600 : 400 }}>
                {s}
              </span>
            ))}
          </div>
      ...
      <div className="hero-float-d absolute bottom-32 left-[5%] z-10">
        <div className="glass-card rounded-sm px-3 py-2">
          <p className="text-[7px] uppercase tracking-wider" style={{ color: "rgba(170,184,200,0.7)" }}>Segmento</p>
          <p className="text-sm font-bold text-ink">B2B · PyMEs</p>
        </div>
      </div>

      {/* Pulsing data dots */}
      <div className="dot-breathe-a absolute top-10 right-6 w-2 h-2 rounded-full" style={{ background: "rgba(110,231,249,0.6)" }} />
      <div className="dot-breathe-b absolute top-24 right-12 w-1.5 h-1.5 rounded-full" style={{ background: "#2E7DC8" }} />
      <div className="dot-breathe-c absolute left-[8%] w-1.5 h-1.5 rounded-full" style={{ bottom: "45%", background: "rgba(110,231,249,0.4)" }} />

      {/* Connecting dotted line */}
      <svg className="absolute inset-0 w-full h-full pointer-events-none" overflow="visible">
        <line x1="30%" y1="62%" x2="60%" y2="78%" stroke="#2E7DC8" strokeWidth="0.75" strokeDasharray="4 4" opacity="0.3" />
      </svg>
    </div>
  );
}
